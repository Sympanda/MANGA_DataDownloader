...
def lookup_drpall_row(plateifu, drpall_path):
    ...
    values = np.array([str(_to_plain_value(x)).strip() for x in tab[plateifu_col]])
    matches = np.where(values == str(plateifu).strip())[0]
    if len(matches) == 0:
        raise ValueError(f"Could not find plateifu={plateifu!r} in {drpall_path}")
    if len(matches) > 1:
        raise ValueError(f"Found multiple rows for plateifu={plateifu!r} in {drpall_path}")

    row = tab[int(matches[0])]
    return {name: _to_plain_value(row[name]) for name in tab.colnames}
...
def build_collaborator_arrays(
    ...
    footprint = np.ones((native_y, native_x), dtype=np.uint8)
    arrays["native_footprint_mask"] = center_pad(footprint, target_shape, pad_value=0)
    ...
    for spec in PIPE3D_MAP_SPECS:
        ...
        arrays[f"{key}_valid_mask"] = np.isfinite(arrays[f"{key}_scaled"]).astype(np.uint8)
        error_key = f"{key}_err"
        error_array_name = None
        if error_key in raw_maps:
            error_array_name = f"{key}_err_raw"
            arrays[error_array_name] = center_pad(raw_maps[error_key], target_shape, pad_value=np.nan)
...
def write_collaborator_maps(
    ...
    arrays, metadata = build_collaborator_arrays(
        pipe3d_path,
